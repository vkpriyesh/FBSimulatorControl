...
@property (nonatomic, assign, readwrite) NSUInteger frameCounter;
...
+ (instancetype)withPath:(NSString *)path logger:(id<FBSimulatorLogger>)logger eventSink:(id<FBSimulatorEventSink>)eventSink
{
  return [[self alloc] initWithPath:path logger:logger eventSink:eventSink];
}

- (instancetype)initWithPath:(NSString *)filePath logger:(id<FBSimulatorLogger>)logger eventSink:(id<FBSimulatorEventSink>)eventSink
{
  self = [super init];
  if (!self) {
    return nil;
...
- (void)drainQueue
{
  while (self.input.readyForMoreMediaData) {
    ...
    if (![self.adaptor appendPixelBuffer:pixelBuffer withPresentationTime:item.time]) {
      [self.logger.error logFormat:@"Failed to append frame %f pixel buffer with error %@", CMTimeGetSeconds(item.time), self.writer.error];
    }
    else if (self.frameCounter % FBFramebufferLogFrameFrequency == 0) {
      [self.logger.debug logFormat:@"Reached Frame %lu", (unsigned long) self.frameCounter];
    }
    self.frameCounter++;
